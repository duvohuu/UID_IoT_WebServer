    ...
    const fetchWorkShifts = async (machineId) => {
        ...
        try {
            ...
            const result = await getWorkShiftsByMachine(machineId, {
                limit: 50,
                page: 1,
                sortBy: 'shiftId', // ✅ Sort theo shiftId
                sortOrder: 'asc'   // ✅ ASC = shiftId nhỏ trước, lớn sau (nằm dưới)
            });
            ...
            if (result.success && result.data?.shifts) {
                ...
                // ✅ DEBUG: Log thứ tự từ backend
                console.log('📋 Work shifts from backend (sorted by shiftId ASC):');
                shifts.forEach((shift, index) => {
                    console.log(`${index + 1}. ${shift.shiftId} - ${new Date(shift.startTime).toLocaleString('vi-VN')}`);
                });
                
                console.log(`✅ Found ${shifts.length} work shifts (backend sorted)`);
                
                setWorkShifts(shifts);
                
                // ✅ Apply current filter (KHÔNG sort lại ở frontend)
                if (shiftFilter === 'all') {
