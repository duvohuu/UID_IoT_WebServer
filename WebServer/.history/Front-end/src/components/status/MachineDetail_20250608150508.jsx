...
const MachineDetail = ({ user }) => {
    ...
    try {
        ...
        const result = await getWorkShiftsByMachine(machineId, {
            limit: 50,
            page: 1
            // ✅ BỎ sortBy và sortOrder - sẽ sort ở frontend
        });
        ...
        if (result.success && result.data?.shifts) {
            ...
            // ✅ THÊM: Sort hoàn toàn ở frontend theo startTime
            shifts = shifts.sort((a, b) => {
                const timeA = new Date(a.startTime).getTime();
                const timeB = new Date(b.startTime).getTime();
                return timeA - timeB; // ✅ ASC: ca cũ nhất trước, ca mới nhất sau
            });
            
            // ✅ DEBUG: Log thứ tự sau khi sort
            console.log('📋 Work shifts sorted by startTime (ASC):');
            shifts.forEach((shift, index) => {
                const startTime = new Date(shift.startTime);
                console.log(`${index + 1}. ${shift.shiftId} - ${startTime.toLocaleString('vi-VN')} (${startTime.getTime()})`);
            });
            
            console.log(`✅ Found ${shifts.length} work shifts (sorted oldest → newest)`);
