...
const MachineDetail = ({ user }) => {
    ...
    const renderUnifiedDataCard = (dataType, config, isAdminOnly = false) => {
        const displayData = getDisplayData(dataType);
         ...
         if (displayData?.shiftInfo && dataType === 'monitoring') {
        console.log('🔍 Efficiency Debug for', displayData.shiftInfo.shiftId, ':', {
            efficiency: displayData.shiftInfo.efficiency,
            totalWeightFilled: displayData.shiftInfo.totalWeightFilled,
            duration: displayData.shiftInfo.duration,
            startTime: displayData.shiftInfo.startTime,
            endTime: displayData.shiftInfo.endTime,
            status: displayData.shiftInfo.status,
            calculation: {
                weightKg: displayData.shiftInfo.totalWeightFilled ? (displayData.shiftInfo.totalWeightFilled / 1000) : 0,
                durationHours: displayData.shiftInfo.duration ? (displayData.shiftInfo.duration / 60) : 0,
                expectedEfficiency: (displayData.shiftInfo.totalWeightFilled && displayData.shiftInfo.duration) 
                    ? ((displayData.shiftInfo.totalWeightFilled / 1000) / (displayData.shiftInfo.duration / 60)).toFixed(2)
                    : 'N/A'
            }
        });
    }
                        ...
                        <Box sx={{ mb: 3, p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
                            <Grid container spacing={2}>
                                ...
                                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                                    ...
                                    {/* ✅ THÊM: Hiển thị giá trị efficiency thực tế */}
                                    <Typography variant="body2" sx={{ fontWeight: 500 }}>
                                        {(() => {
                                            const eff = displayData.shiftInfo.efficiency;
                                            if (eff === null || eff === undefined) return 'Chưa có dữ liệu';
                                            if (eff === 0) return '0.00 kg/h';
                                            if (eff > 0) return `${Number(eff).toFixed(2)} kg/h`;
                                            return 'Lỗi dữ liệu';
                                        })()}
                                    </Typography>
                                    {/* ✅ GIỮ: Debug info */}
                                    <Typography variant="caption" color="text.secondary" sx={{ display: 'block', fontSize: '0.7rem' }}>
                                        Raw: {displayData.shiftInfo.efficiency || 'null'} | Weight: {displayData.shiftInfo.totalWeightFilled || 0}g | Duration: {displayData.shiftInfo.duration || 0}min
                                    </Typography>
                                </Grid>
                            </Grid>
                        </Box>
