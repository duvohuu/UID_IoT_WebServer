...
workShiftSchema.pre('save', function(next) {
    ...
    if (this.totalWeightFilled && this.duration && this.duration > 0) {
        const weightInKg = this.totalWeightFilled / 1000; // Convert grams to kilograms
        const durationInHours = this.duration / 60; // Convert minutes to hours
        this.efficiency = Number((weightInKg / durationInHours).toFixed(2)); // kg/hour, rounded to 2 decimal places
    } else {
        this.efficiency = 0; // Set efficiency to 0 if duration is 0 or no weight filled
    }
