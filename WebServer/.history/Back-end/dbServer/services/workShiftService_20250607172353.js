...
class WorkShiftService {
    ...
    async checkAndCreateShiftByID(machine, currentParameters, currentTime) {
        ...
        // ✅ Tạo shift ID string
        const shiftId = `SHIFT_${machine.machineId}_${shiftNumericId}`;
        
        console.log(`🔍 [${machine.name}] Checking shift ID: ${shiftId} (Low: ${shiftIdLow}, High: ${shiftIdHigh})`);
        ...
        try {
            ...
            if (existingShift) {
                ...
                console.log(`📝 [${machine.name}] Shift exists - updating data: ${shiftId}`);
    ...
    async createNewShiftFromData(machine, shiftId, currentParameters, currentTime) {
        console.log(`🟢 [${machine.name}] Creating new shift: ${shiftId}`);
        ...
        await newShift.save();
        console.log(`✅ [${machine.name}] Shift created: ${shiftId} (${status})`);
        ...
        try {
            ...
            console.log(`📡 [${machine.name}] Shift notification sent (${status})`);
        } catch (notifyError) {
