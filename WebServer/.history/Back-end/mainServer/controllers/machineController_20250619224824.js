...
export const deleteMachine = async (req, res) => {
    try {
        ...
        console.log('✅ MAIN SERVER: Response from DB Server:');
        console.log(`   📊 Deleted machine: ${response.data.deletedMachine.machineId} - ${response.data.deletedMachine.name}`);
        console.log(`   📊 Deleted work shifts: ${response.data.deletedShiftsCount} total`);
        ...
        res.status(500).json({ 
            ...
            error: error.message,
            details: error.response?.data?.message || 'DB Server communication error'
        });
    }
};
